      ...
      <div className="px-4 space-y-4">
        ...
        {step === 1 && (
          <>
            ...
            {/* Only post-to-pre has an actual choice here (Prepaid or Basic Postpaid);
                pre-to-post only ever migrates to Postpaid, so there's no real toggle to
                show — the whole section is skipped rather than displaying a one-tile
                "choice". Sizing matches SIM Activation's own 2+-option Subscription Type
                tiles (scaled down a notch from the single-option size). */}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([
                    ...
                    const selected = targetSubType === value;
                    return (
                      <button
                        key={value}
                        type="button"
                        onClick={() => setTargetSubType(value)}
                        className={cn(
                          "relative min-w-0 flex flex-col items-center justify-center gap-1.5 rounded-2xl py-3.5 px-1.5 transition-all",
                          selected ? "border-[0.5px] bg-primary/10 border-primary/20" : "border bg-card border-border/60"
                        )}
                      >
                        <span className={cn(
                          "absolute top-2 end-2 w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center",
                          selected ? "border-primary bg-primary" : "border-muted-foreground/30"
                        )}>
                          {selected && <span className="w-1 h-1 rounded-full bg-primary-foreground" />}
                        </span>
                        <Icon className={cn("w-5 h-5", selected ? "text-primary" : "text-muted-foreground")} />
                        <p className={cn("text-xs font-semibold text-center leading-snug", selected ? "text-foreground" : "text-muted-foreground")}>
                          {label}
                        </p>
                      </button>
                    );
