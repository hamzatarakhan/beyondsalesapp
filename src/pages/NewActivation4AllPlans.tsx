...
interface NavState {
  ...
  // the round trip so they survive /new-activation-4 remounting on return.
  resume?: unknown;
}

const NewActivation4AllPlans = () => {
  ...
  const pickPlan = (title: string) => {
    navigate("/new-activation-4", { state: { pickPlan: { payType, lineType, chip: effectiveChip, title }, resume: initial.resume } });
  };
      ...
      <div className="sticky top-0 z-10 bg-background">
        ...
        <div className="px-4 flex items-center gap-2 pb-4">
          ...
          <button
            ...
            aria-label={t("activation4.subscription.filtersAria")}
      ...
      <div className="px-4 space-y-3">
        {visiblePlans.length === 0 ? (
          <div className="bg-card rounded-2xl p-6 text-center text-sm text-muted-foreground shadow-sm">
            {t("activation4.subscription.noPlansMatchSearch")}
          </div>
        ) : (
          visiblePlans.map((p) => {
      ...
      <Drawer open={filterOpen} onOpenChange={setFilterOpen}>
        <DrawerContent className="bg-card rounded-t-3xl max-h-[85vh]">
          ...
          <DrawerHeader className="text-center pt-8">
            <DrawerTitle className="text-lg font-semibold">{t("activation4.subscription.filtersTitle")}</DrawerTitle>
          </DrawerHeader>
          <div className="px-4 pb-8 space-y-5 overflow-y-auto">
            {!isFriendi && (
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {([
                    { key: "mobile" as const, label: t("activation4.subscription.lineMobile") },
                    { key: "data" as const, label: t("activation4.subscription.lineData") },
                  ]).map(({ key, label }) => (
                    <button
                      key={key}
                      type="button"
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2">
                {(isFriendi
                  ? [
                      { key: "prepaid" as const, label: t("activation4.subscription.prepaid") },
                      { key: "basic-postpaid" as const, label: t("activation4.subscription.basicPostpaid") },
                    ]
                  : [
                      { key: "prepaid" as const, label: t("activation4.subscription.prepaid") },
                      { key: "postpaid" as const, label: t("activation4.subscription.postpaid") },
                      { key: "basic-postpaid" as const, label: t("activation4.subscription.basicPostpaid") },
                    ]
                ).map(({ key, label }) => (
                  <button
            ...
            >
              {t("activation4.subscription.applyFilters")}
            </button>
          </div>
        </DrawerContent>
      </Drawer>
...
export default NewActivation4AllPlans;
