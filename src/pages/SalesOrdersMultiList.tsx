...
import { Search, SlidersHorizontal, X as XIcon, Clock, Boxes, User, Plus } from "lucide-react";
import { salesOrdersMulti, PURCHASE_ORDER_PRODUCTS, DEMO_CHANNEL_MEMBERS, type SalesOrderStatus } from "@/data/salesOrdersMultiStore";

// Identical list UI to the single-location Sales Orders list — the Multiple Locations
// business only differs from "Select Location" onward inside View Order.
...
const SalesOrdersMultiList = () => {
  const navigate = useNavigate();
  const { t } = useTranslation();

  const [search, setSearch] = useState("");
  const [filterOpen, setFilterOpen] = useState(false);
  const [draftStatus, setDraftStatus] = useState<SalesOrderStatus | null>(null);
  const [draftMemberCode, setDraftMemberCode] = useState<string>("");
  const [appliedStatus, setAppliedStatus] = useState<SalesOrderStatus | null>(null);
  const [appliedMemberCode, setAppliedMemberCode] = useState<string>("");
  ...
  const activeFilterCount = (appliedStatus ? 1 : 0) + (appliedMemberCode ? 1 : 0) + (appliedRange ? 1 : 0);

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase();
    return salesOrdersMulti.filter((o) => {
  ...
  const openFilter = () => {
    ...
    setDraftMemberCode(appliedMemberCode);
    setFilterOpen(true);
  };

  const applyFilter = () => {
    setAppliedStatus(draftStatus);
    setAppliedMemberCode(draftMemberCode);
    setFilterOpen(false);
  };
  ...
  return (
    <div className="mobile-container min-h-screen bg-background pb-24">
      <AppHeader title={t("salesOrders.title")} showBack />
      ...
      <div className="px-4 space-y-3">
        ...
        {(appliedStatus || appliedMemberCode || appliedRange) && (
          <div className="flex items-center gap-2 flex-wrap">
            {appliedStatus && (
              <span className="inline-flex items-center gap-1.5 ps-3 pe-2 py-1 rounded-full bg-primary/10 text-primary text-xs font-medium">
                {t(`salesOrders.status.${appliedStatus}`)}
            ...
            {appliedMemberCode && (
              <span className="inline-flex items-center gap-1.5 ps-3 pe-2 py-1 rounded-full bg-primary/10 text-primary text-xs font-medium">
                {DEMO_CHANNEL_MEMBERS.find((m) => m.code === appliedMemberCode)?.name}
                <button type="button" onClick={() => setAppliedMemberCode("")} aria-label={t("purchaseOrders.removeFilter")} className="w-4 h-4 rounded-full flex items-center justify-center">
                  <XIcon className="w-3 h-3" />
                </button>
              </span>
            )}
        ...
        {filtered.length === 0 ? (
          <div className="py-16 text-center text-sm text-muted-foreground">{t("salesOrders.noOrders")}</div>
        ) : (
          <div className="space-y-3">
            {filtered.map((o) => (
              <button
                key={o.id}
                type="button"
                onClick={() => navigate(`/sales-orders-multi/${o.id}`)}
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className={cn("px-2.5 py-0.5 rounded-full text-[10px] font-semibold shrink-0", STATUS_STYLE[o.status])}>{t(`salesOrders.status.${o.status}`)}</span>
                </div>
      ...
      <button
        type="button"
        onClick={() => navigate("/sales-orders-multi/new")}
        ...
        <DrawerContent className="bg-card rounded-t-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="px-4 pb-6 space-y-5">
            ...
            <div className="space-y-1.5">
              <p className="text-xs font-semibold text-foreground px-1">{t("salesOrders.channelMember")}</p>
              <Select value={draftMemberCode || "all"} onValueChange={(v) => setDraftMemberCode(v === "all" ? "" : v)}>
                <SelectTrigger className="h-11 rounded-xl">
                  <SelectValue placeholder={t("salesOrders.selectChannelMember")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">{t("purchaseOrders.filter.all")}</SelectItem>
                  {DEMO_CHANNEL_MEMBERS.map((m) => (
...
export default SalesOrdersMultiList;
