...
// Same fake-capture convention as PurchaseOrderScan.tsx — no real camera, the capture
// button just picks a demo channel member.
...
const SalesOrderForm = () => {
  ...
  useEffect(() => {
    if (isEdit && !existing) navigate("/sales-orders", { replace: true });
  ...
  const backToList = () => navigate(`/sales-orders${location ? `?location=${encodeURIComponent(location)}` : ""}`);
  ...
  if (step === 0) {
    return (
      <div className="mobile-container min-h-screen bg-background pb-28">
        ...
        {/* ---------- Fake camera viewfinder (same look as PurchaseOrderScan.tsx) ---------- */}
        {cameraOpen && (
  ...
  // ---------- Step 1: Destination + products (identical to Purchase Order's Create/Edit) ----------
  return (
    <div className="mobile-container min-h-screen bg-background pb-28">
