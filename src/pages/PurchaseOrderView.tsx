...
import { PURCHASE_ORDER_PRODUCTS, getPurchaseOrder, updatePurchaseOrder, type ProductId, type PurchaseOrderStatus } from "@/data/purchaseOrdersStore";
      ...
      <div className="px-4 space-y-3">
        <div className="bg-card rounded-2xl p-4 shadow-sm space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className={cn("px-2.5 py-0.5 rounded-full text-[10px] font-semibold shrink-0", STATUS_STYLE[order.status])}>{t(`purchaseOrders.status.${order.status}`)}</span>
          </div>
          ...
          <p className="text-xs text-muted-foreground px-1">{order.date}</p>
        </div>
              ...
              {order.status === "awaitingScanning" && (
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  {isFullyScanned ? (
                    ...
                    <button type="button" onClick={() => navigate(`/purchase-orders/${order.id}/scan/${l.productId}`)} aria-label={t("purchaseOrders.scanAria")} className="w-8 h-8 rounded-lg border border-border flex items-center justify-center">
