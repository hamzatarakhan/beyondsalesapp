...
import { getSalesOrderMulti, updateSalesOrderMulti, PURCHASE_ORDER_PRODUCTS, type ProductId } from "@/data/salesOrdersMultiStore";

// Same fake-capture convention as the single-location Scan ID page — here it scans one
// specific source within a line, since Multiple Locations can split one product's qty
// across several source locations that each need their own serials.
...
const SalesOrdersMultiScan = () => {
  const navigate = useNavigate();
  const { t } = useTranslation();
  const { id, productId, sourceIndex } = useParams<{ id: string; productId: ProductId; sourceIndex: string }>();
  const order = id ? getSalesOrderMulti(id) : undefined;
  const line = order?.lines.find((l) => l.productId === productId);
  const srcIdx = Number(sourceIndex ?? -1);
  const src = line?.sources[srcIdx];
  ...
  const remaining = src ? Math.max(0, src.qty - src.scanned) : 0;
  ...
  const submit = () => {
    if (!order || !line || !src || !productId) return;
    ...
    const mergedSerials = [...src.serials, ...newSerials].slice(0, src.qty);
    updateSalesOrderMulti(order.id, {
      lines: order.lines.map((l) =>
        l.productId === productId
          ? { ...l, sources: l.sources.map((s, i) => (i === srcIdx ? { ...s, serials: mergedSerials, scanned: mergedSerials.length } : s)) }
          : l,
      ),
    });
...
export default SalesOrdersMultiScan;
