...
const SalesOrders = () => {
  ...
  return (
    <div className="mobile-container min-h-screen bg-background pb-24">
      ...
      {location && (
        <div className="px-4 pb-3 -mt-1">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-muted text-[11px] font-semibold text-muted-foreground">
            <MapPin className="w-3 h-3" /> {t("salesOrders.viewingLocation", { location })}
          </span>
        </div>
      )}
      ...
      <button
        type="button"
        onClick={() => navigate(`/sales-orders/new${location ? `?location=${encodeURIComponent(location)}` : ""}`)}
      ...
      {/* ---------- Pick a date drawer — full-width calendar, same as Purchase Orders' ---------- */}
