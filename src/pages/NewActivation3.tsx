                            ...
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              {showCommitted && pickedVanityCat ? (
                                ...
                                <span className="text-[11px] font-semibold text-foreground">
                                  {pickedVanityCat ? <><RiyalSymbol /> {pickedVanityCat.price}</> : (tab.fee ? <><RiyalSymbol /> {tab.fee}</> : t("activation3.checkout.free"))}
                                </span>
                              )}
                            </div>
                            {showCommitted && pickedVanityCat && (
            ...
            {isPaygPlan && selectedPlanObj && (() => {
              ...
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                ...
                <p className="text-[11px] text-muted-foreground mb-3">
                  {hasSelection
                    ? <><RiyalSymbol /> {inclVat.toFixed(1)} {t("activation3.subscription.topupInclVat")}</>
            ...
            {showNumber && (
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                ...
                <div className="mb-3">
                  ...
                  {isMnpIneligiblePlan && (
                    <p className="text-[11px] text-muted-foreground mt-1.5 px-1">{t("activation3.subscription.mnpTabDisabledHint")}</p>
                  )}
                </div>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[11px] font-semibold" style={{ color: tab.color ?? undefined }}>{t(`activation.subscription.numberTabs.${tab.value}`, tab.label)}</span>
                            <span className="text-[11px] text-muted-foreground">·</span>
                            <span className="text-[11px] font-semibold text-foreground">{tab.fee ? <><RiyalSymbol /> {tab.fee}</> : t("activation3.checkout.free")}</span>
                          </div>
                        );
                      })()}
                      */}
                        ...
                        >
                          ...
                          <span className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-muted-foreground">
                              {vanityCommitment
                                ? t("activation3.vanity.commitmentOn", { months: pickedVanityCat.months })
                                : t("activation3.vanity.commitmentOff", { price: pickedVanityCat.price })}
                            </p>
                          </span>
                      ...
                      pickedCategoryEligibleFree ? (
                        <div className="mt-3 rounded-xl bg-muted/40 border border-border/60 p-3 space-y-2.5">
                          ...
                          {vanityCommitment
                            ...
                            : <p className="text-[11px] font-semibold text-foreground"><RiyalSymbol /> {pickedVanityCat.price} · {t("activation3.vanity.noCommitment")}</p>}
                        </div>
                      ) : (
                        <div className="mt-3 rounded-xl bg-muted/40 border border-border/60 p-3 space-y-1">
                          <p className="text-[13px] font-semibold text-foreground">{t(`activation.vanity.categories.${pickedVanityCat.key}`)} · {t(`activation.vanity.tiers.${pickedVanityCat.tier}`)}</p>
        ...
        {step === 2 && (
          <>
            ...
            <section className="bg-card rounded-2xl p-4 shadow-sm">
              ...
              {(planMode === "topup" || isPaygPlan) && topupAmount > 0 && <SummaryRow label={t("activation3.checkout.topupValue")} value={<><RiyalSymbol /> {topupAmount}</>} />}
                    ...
                    {activePromo.benefits.map((b, i) => {
                      if (b.type === "discount") return (
                        <span key={i} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md border border-emerald-200/70 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-medium">
                          <RiyalSymbol className="w-2.5 h-2.5" /> {b.value} {t("activation3.checkout.promoDiscount")}
                        </span>
                      );
            ...
            {!(isWhitelisted && !isVipNumber) && !(isFulfilment && alreadyPaid) && total > 0 && (
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                ...
                <div className="space-y-2">
                  <PayOption icon={CreditCard} label={t("activation3.checkout.dealerWallet")} description={t("activation3.checkout.dealerWalletDesc", { balance: DEALER_WALLET_BALANCE })} selected={pay === "card"} onClick={() => setPay("card")} />
        ...
        <div className="max-w-[390px] mx-auto">
          ...
          {step < 2 ? (
            ...
            <Button className="w-full h-12 text-sm font-semibold rounded-full" disabled={!canPay} onClick={() => setPayConfirmOpen(true)}>
              {total === 0 ? t("activation3.checkout.submit") : <>{t("activation3.checkout.pay")} <RiyalSymbol /> {total}</>}
            </Button>
          )}
        </div>
      </div>
          ...
          <Dialog open onOpenChange={(o) => !o && setPendingVanityNumber(null)}>
            <DialogContent className="max-w-[320px] rounded-3xl border-0 p-6 text-center [&>button]:hidden">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  {t("activation3.vanity.payNumberPrice", { price: cat.price })}
                </button>
