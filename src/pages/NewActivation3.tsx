  ...
  return (
    <div className="rounded-xl bg-emerald-50 dark:bg-emerald-900/20 px-3 py-2 flex items-center gap-2">
      ...
      <p className="flex-1 text-[13px] font-medium text-emerald-600 dark:text-emerald-400">
        {label ?? t("activation3.checkout.verifiedTitle")}
      </p>
      {onRetry && (
...
const NewActivation3 = () => {
  ...
  const lineTypeOptions: { key: LineType; label: string; Icon: typeof Wallet; disabled?: boolean }[] = [
    ...
    { key: "data", label: t("activation3.subscription.lineData"), Icon: Wifi, disabled: dataLineUnavailable },
  ];
  ...
  // E-SIM is selected, so Postpaid/Basic Postpaid are dropped from the row entirely
  // (rather than shown disabled) and Prepaid takes the full row width on its own.
  ...
  const step1Missing = useMemo(() => {
    ...
    if (isPaygPlan && topupRequired && (topupDenom == null || topupDenom < 10)) missing.push(t("activation3.missing.topupMin"));
    if (showMnp && subType === "mnp") {
      if (!portNumber) missing.push(t("activation3.subscription.portNumber"));
      if (!portOperator) missing.push(t("activation3.subscription.currentOperator"));
      if (!portContact) missing.push(t("activation3.missing.portContact"));
    }
    return missing;
  ...
  const step2Missing = useMemo(() => {
    const missing: string[] = [];
    if (emailRequired && !contactEmail.trim()) missing.push(t("activation3.checkout.email"));
    if (cityRequired && !contactCity.trim()) missing.push(t("activation3.subscription.city"));
    if (contactNumberRequired && !contactNumber.trim()) missing.push(t("activation3.checkout.contactNumber"));
    if (isVnetMode && !nationalAddress.trim()) missing.push(t("activation3.subscription.nationalAddress"));
    if (showDelivery && !deliveryAddress.trim()) missing.push(t("activation3.checkout.addressLine"));
    if (showHandoverOption && isDealerHandover && !deviceSerialNumber.trim()) missing.push(t("activation3.handover.deviceSerialNumber"));
    if (!customerVerified) missing.push(t("activation3.checkout.customerVerification"));
    if (otpRequired && !otpVerified) missing.push(t("activation3.checkout.otp"));
    if (showNafith && !nafithVerified) missing.push(t("activation3.checkout.nafath"));
  ...
  return (
    <div className="mobile-container bg-background min-h-screen pb-32">
      ...
      <div className="px-4 space-y-4">
        ...
        {step === 0 && (
          <>
            {!isFulfilment ? (
              <>
                <Field label={t("activation3.identity.idType")}>
                  <Select value={idType} onValueChange={(v) => { setIdType(v); if (v === "saudi-id") setNationality("sa"); setIdNumber(demoIdFor(ID_TYPE_RULES[v], NORMAL_TEST_ID_SUFFIX)); }}>
                    <SelectTrigger className="w-full bg-card rounded-xl h-12">
                      <SelectValue placeholder={t("activation3.identity.idType")} />
                    </SelectTrigger>
                ...
                <Field label={t(`activation.identity.idFieldLabels.${idNumberRule?.fieldLabelKey ?? "idNumber"}`)}>
                  <Input
                    ...
                    placeholder={t("activation3.identity.idPlaceholder")}
                  ...
                  {idNumber.trim().length > 0 && !idNumberValid && idNumberRule && (
                    <p className="text-xs text-destructive">
                      {idNumberRule.startDigits
                        ? t("activation3.identity.idNumberErrors.startAndLength", { digits: idNumberRule.startDigits.join(", "), length: idNumberRule.length })
                        : t("activation3.identity.idNumberErrors.lengthOnly", { length: idNumberRule.length })}
                    </p>
                  )}
                </Field>
              </>
              ...
              <>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-foreground">{t("activation3.identity.scanQr")}</p>
                    <p className="text-[10px] text-muted-foreground mt-0.5">{t("activation3.identity.scanQrNote")}</p>
                  </div>
        ...
        {step === 1 && (
          <>
            ...
            <section>
                ...
                {simType === "esim" && (
                  <button type="button" onClick={() => setEsimInfoOpen(true)} className="w-full mt-3 flex items-center gap-3 text-start p-3.5 rounded-2xl bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/25 hover:border-primary/50 transition-all group">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold text-foreground">{t("activation3.subscription.esimSupportedDevices")}</p>
                      <p className="text-[10px] text-muted-foreground mt-0.5">{t("activation3.subscription.esimSupportedNote")}</p>
                    </div>
                    <ArrowRight className="w-3.5 h-3.5 text-primary/60 shrink-0 rtl:rotate-180" />
                  </button>
                )}
                ...
                {showNumber && subType === "sim" && phone && (
                  <section className="bg-card rounded-2xl p-4 shadow-sm">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-sm font-semibold text-foreground">{t("activation3.subscription.phoneSection")}</p>
                    </div>
                            ...
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              {showCommitted && pickedVanityCat ? (
                                ...
                                <span className="text-[11px] font-semibold text-foreground">
                                  {pickedVanityCat ? <><RiyalSymbol /> {pickedVanityCat.price.toFixed(2)}</> : (tab.fee ? <><RiyalSymbol /> {tab.fee.toFixed(2)}</> : t("activation3.checkout.free"))}
                                </span>
                              )}
                            </div>
                            {showCommitted && pickedVanityCat && (
                              <span className="text-[11px] font-semibold text-emerald-600">
                                {t("activation3.vanity.commitmentOn", { months: pickedVanityCat.months })}
                              </span>
                            )}
                          </div>
                        );
                ...
                {showNumber && subType === "mnp" && portNumber && (
                  <section className="bg-card rounded-2xl p-4 shadow-sm">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-sm font-semibold text-foreground">{t("activation3.subscription.portMnp")}</p>
                    </div>
                    <div className="bg-primary/5 rounded-xl py-3 px-4 flex flex-col items-center gap-1">
                      ...
                      {subscriptionOptions.map(({ key, label, Icon, disabled }) => (
                        <button
                          ...
                          className={cn(
                            ...
                            compact ? "py-2.5 px-1" : "py-3 px-1.5",
                            payType === key && !disabled ? "border-[0.5px] bg-primary/10 border-primary/20" : "border bg-card border-border/60",
                            disabled && "opacity-40 cursor-not-allowed",
                          )}
                        >
                          <Icon className={cn(compact ? "w-4 h-4" : "w-5 h-5", payType === key && !disabled ? "text-primary" : "text-muted-foreground")} />
                          <p className={cn(
                            "font-medium text-center leading-tight",
                            compact ? "text-[10.5px]" : "text-xs",
                            payType === key && !disabled ? "text-primary" : "text-foreground",
                          )}>
                            {label}
                          </p>
                        </button>
            ...
            {showPlanTypeChips && (
              ...
              >
                {activePlanChips.map(chip => (
                  ...
                  >
                    {({
                      "all": t("activation3.subscription.chips.all"),
                      "flex": t("activation3.subscription.chips.flex"),
                      "aman": t("activation3.subscription.chips.aman"),
                      "base-plan": t("activation3.subscription.chips.baqa"),
                      "data": isFriendi ? t("activation3.subscription.chips.internet") : t("activation3.subscription.chips.data"),
                      "switch-postpaid": t("activation3.subscription.chips.switchPostpaid"),
                      "vnet": t("activation3.subscription.chips.vnet"),
                      "combo": t("activation3.subscription.chips.combo"),
                      "flexi": t("activation3.subscription.chips.flexiPlans"),
                      "calls": t("activation3.subscription.chips.international"),
                      "payg": t("activation3.subscription.chips.payg"),
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder={t("activation3.subscription.searchPlans")}
              ...
              <div className="rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200/70 dark:border-amber-500/25 p-3 flex items-start gap-2.5">
                ...
                <p className="text-[12.5px] font-medium text-foreground">
                  {mnpIneligibleReason === "lowTier"
                    ? t("activation3.subscription.mnpNotEligibleLowTier", { minPrice: MNP_MIN_PLAN_PRICE[payType === "postpaid" ? "postpaid" : "prepaid"] })
                    : t("activation3.subscription.mnpNotEligible")}
                </p>
              </div>
            )}
            ...
            {isPaygPlan && selectedPlanObj && (() => {
              ...
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">{t("activation3.subscription.topupSectionTitle")}</p>
                    <p className="text-[11px] text-muted-foreground">{t("activation3.subscription.topupChooseSub")}</p>
                  </div>
                </div>
                <div className="h-12 rounded-xl border border-border/60 flex items-center justify-center mb-2">
                  {hasSelection
                    ...
                    : <span className="text-sm text-muted-foreground">{t("activation3.subscription.topupSelectAmount")}</span>}
                </div>
                <p className="text-[11px] text-muted-foreground mb-3">
                  {hasSelection
                    ? <><RiyalSymbol /> {inclVat.toFixed(2)} {t("activation3.subscription.topupInclVat")}</>
                    : t("activation3.subscription.topupVatNote")}
                </p>
            ...
            {showDevice && deviceObj && (
              <section>
                <h3 className="text-sm font-semibold text-foreground mb-2">
                  {t("activation3.subscription.deviceTitle")} <span className="text-destructive">*</span>
                </h3>
                <div className="w-full flex items-center gap-3 p-3.5 rounded-2xl border border-primary/20 bg-primary/5">
                  ...
                  <div className="text-right shrink-0">
                    {deviceObj.price > 0
                      ...
                      : <span className="text-xs font-semibold text-green-600 bg-green-50 px-2 py-0.5 rounded-full">{t("activation3.subscription.deviceIncluded")}</span>
                    }
                  </div>
                </div>
              </section>
              ...
              <div className="rounded-2xl border border-border/60 bg-card overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-foreground">{t("activation3.handover.title")}</p>
                      <p className="text-[11px] text-muted-foreground">{t("activation3.handover.subtitle")}</p>
                    </div>
                  </div>
                ...
                {isDealerHandover && (
                  <div className="px-4 pb-4 pt-1 border-t border-border/60">
                    <Field label={`${t("activation3.handover.deviceSerialNumber")} *`}>
              ...
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-base font-bold text-foreground">
                    {t("activation3.subscription.phoneSection")} <span className="text-destructive">*</span>
                  </p>
                </div>
                ...
                <div className="mb-3">
                  <div className="grid grid-cols-2 gap-2">
                    <SourceTab
                      active={subType === "sim"}
                      icon={Sparkles}
                      label={t("activation3.subscription.newNumberBtn")}
                      variant="filled"
                    ...
                    <SourceTab
                      active={subType === "mnp"}
                      icon={ArrowRightLeft}
                      label={t("activation3.subscription.portMnp")}
                      variant="filled"
                  ...
                  return (
                    <div className="mb-3 rounded-2xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200/70 dark:border-amber-500/25 p-3 flex items-center gap-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-[12.5px] font-semibold text-foreground">{t("activation3.vanity.availableBannerTitle")}</p>
                ...
                {subType === "sim" ? (
                  <>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[11px] font-semibold" style={{ color: tab.color ?? undefined }}>{t(`activation.subscription.numberTabs.${tab.value}`, tab.label)}</span>
                            <span className="text-[11px] text-muted-foreground">·</span>
                            <span className="text-[11px] font-semibold text-foreground">{tab.fee ? <><RiyalSymbol /> {tab.fee.toFixed(2)}</> : t("activation3.checkout.free")}</span>
                          </div>
                        );
                      })()}
                      */}
                    ...
                    <button onClick={() => setNumberPickerOpen(true)} className="w-full flex items-center justify-center gap-1.5 text-link text-sm font-semibold">
                      {t("activation3.subscription.pickDifferent")} <ArrowRight className="w-4 h-4 rtl:rotate-180" />
                    </button>
                    {SHOW_VANITY_OVERVIEW && isPostpaidMobile && !selectedPlanObj && (
                      <p className="text-[11px] text-muted-foreground text-center mt-2">{t("activation3.vanity.selectPlanFirst")}</p>
                    )}
                      ...
                      pickedCategoryEligibleFree ? (
                        ...
                        >
                          ...
                          <span className="flex-1 min-w-0">
                            <p className="text-[13px] font-semibold text-foreground">{t("activation3.vanity.commitmentLabel")}</p>
                            <p className="text-[11px] text-muted-foreground">
                              {vanityCommitment
                                ? t("activation3.vanity.commitmentOn", { months: pickedVanityCat.months })
                                : t("activation3.vanity.commitmentOff", { price: pickedVanityCat.price.toFixed(2) })}
                            </p>
                          </span>
                          <span className="text-[10px] font-semibold text-emerald-700 bg-emerald-50 dark:bg-emerald-500/10 rounded-full px-2 py-1 shrink-0">
                            {t("activation3.vanity.monthsPill", { months: pickedVanityCat.months })}
                          </span>
                        </button>
                      ) : (
                        <div className="mt-3 rounded-xl bg-muted/40 border border-border/60 p-3 space-y-1">
                          ...
                          <p className="text-[11px] text-muted-foreground">{t("activation3.vanity.notEligible")}</p>
                  ...
                  <div className="space-y-3">
                    <Field label={t("activation3.subscription.portNumber")}><PhoneNumberInput value={portNumber} onChange={setPortNumber} /></Field>
                    <Field label={t("activation3.subscription.currentOperator")}>
                      <Select value={portOperator} onValueChange={setPortOperator}>
                        <SelectTrigger><SelectValue placeholder={t("activation3.subscription.selectOperator")} /></SelectTrigger>
                        <SelectContent>{OPERATORS.map((o) => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent>
                      </Select>
                    </Field>
                    <p className="text-[11px] text-muted-foreground">{t("activation3.subscription.portNote")}</p>
                  </div>
                )}
                ...
                {SHOW_VANITY_OVERVIEW && isPostpaidMobile && subType === "sim" && selectedPlanObj && (
                  <div className="mt-4 pt-3 border-t border-border/60">
                    <p className="text-xs font-semibold text-foreground">{t("activation3.vanity.title")}</p>
                    <p className="text-[11px] text-muted-foreground mb-2.5">{t("activation3.vanity.subtitle")}</p>
                    <div className="space-y-2">
                        ...
                        >
                          <div className="flex items-center gap-2.5 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-[11px] text-muted-foreground">
                                {c.months > 0 ? t("activation3.vanity.commitment", { months: c.months }) : t("activation3.vanity.noCommitment")}
                              </p>
                            </div>
                          </div>
                          <span className="flex items-center gap-1 shrink-0 ms-2">
                            <span className="text-[11px] font-semibold text-emerald-600">
                              {c.months > 0 ? t("activation3.vanity.freeWithCommitment") : t("activation3.vanity.alwaysFree")}
                            </span>
                    ...
                    {SHOW_VANITY_OVERVIEW && pickedVanityCat && pickedVanityCat.months > 0 && (
                      pickedCategoryEligibleFree ? (
                        <div className="mt-3 rounded-xl bg-muted/40 border border-border/60 p-3 space-y-2.5">
                          ...
                          {vanityCommitment
                            ? <p className="text-[11px] text-primary flex items-start gap-1.5"><FileText className="w-3.5 h-3.5 mt-0.5 shrink-0" /> {t("activation3.vanity.nafithNote")}</p>
                            : <p className="text-[11px] font-semibold text-foreground"><RiyalSymbol /> {pickedVanityCat.price.toFixed(2)} · {t("activation3.vanity.noCommitment")}</p>}
                        </div>
                      ) : (
                        <div className="mt-3 rounded-xl bg-muted/40 border border-border/60 p-3 space-y-1">
                          <p className="text-[13px] font-semibold text-foreground">{t(`activation.vanity.categories.${pickedVanityCat.key}`)} · {t(`activation.vanity.tiers.${pickedVanityCat.tier}`)}</p>
          ...
          <>
            ...
            <section className="bg-card rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-foreground">{t("activation3.checkout.summary")}</p>
                </div>
              </div>
              ...
              {selectedPlanObj && (() => {
                ...
                const planTypeLabel =
                  cats.includes("switch-postpaid") ? t("activation3.subscription.chips.switchPostpaid") :
                  cats.includes("vnet") ? t("activation3.subscription.chips.vnet") :
                  cats.includes("payg") ? t("activation3.subscription.chips.payg") :
                  cats.includes("combo") ? t("activation3.subscription.chips.combo") :
                  cats.includes("flexi") ? t("activation3.subscription.chips.flexiPlans") :
                  cats.includes("calls") ? t("activation3.subscription.chips.international") :
                  cats.includes("data") ? (isFriendi ? t("activation3.subscription.chips.internet") : t("activation3.subscription.chips.data")) :
                  cats.includes("aman") ? t("activation3.subscription.chips.aman") :
                  cats.includes("base-plan") ? t("activation3.subscription.chips.baqa") :
                  cats.includes("flex") ? t("activation3.subscription.chips.flex") : "";
                return planTypeLabel ? <SummaryRow label={t("activation3.checkout.planType")} value={planTypeLabel} /> : null;
              })()}
              {selectedPlanObj && <SummaryRow label={t("activation3.checkout.planName")} value={selectedPlanObj.title} />}
              {selectedPlanObj?.validityLabel && <SummaryRow label={t("activation3.checkout.planValidity")} value={formatValidity(selectedPlanObj.validityLabel)} />}
              {(planMode === "topup" || isPaygPlan) && topupAmount > 0 && <SummaryRow label={t("activation3.checkout.topupValue")} value={<><RiyalSymbol /> {topupAmount.toFixed(2)}</>} />}
              {showNumber && <SummaryRow label={t("activation3.checkout.numberType")} value={subType === "sim" ? t("activation3.subscription.newNumberBtn") : t("activation3.subscription.portMnp")} />}
              {showNumber && subType === "sim" && phone && <SummaryRow label={t("activation3.checkout.phoneNumber")} value={phone} />}
              {showNumber && subType === "sim" && pickedTier && pickedTier !== "standard" && (
                <SummaryRow label={t("activation3.checkout.vanityLevel")} value={t(`activation.vanity.tiers.${pickedTier}`, NUMBER_TABS.find(tb => tb.value === pickedTier)?.label ?? "")} />
              )}
              {showNumber && subType === "mnp" && portNumber && <SummaryRow label={t("activation3.subscription.portNumber")} value={portNumber} />}
              {showDevice && <SummaryRow label={t("activation3.checkout.device")} value={deviceObj?.name ?? ""} />}
            </section>
            ...
            <section className="bg-card rounded-2xl p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm font-semibold text-foreground">{t("activation3.checkout.promoCode")}</p>
                </div>
                <Switch
              ...
              {promoEnabled && (promoApplied && activePromo ? (
                <div className="space-y-2">
                  ...
                  <div className="mt-3 flex items-center justify-between p-3 rounded-xl bg-green-50 border border-green-200">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold text-green-700">{promoCode} {t("activation3.checkout.promoApplied")}</span>
                    </div>
                    <button type="button" onClick={() => { setPromoApplied(false); setPromoCode(""); setPromoError(false); }} className="text-xs text-muted-foreground hover:text-destructive font-medium shrink-0">{t("activation3.checkout.removePromo")}</button>
                  </div>
                    ...
                    {activePromo.benefits.map((b, i) => {
                      if (b.type === "discount") return (
                        <span key={i} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md border border-emerald-200/70 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-medium">
                          <RiyalSymbol className="w-2.5 h-2.5" /> {b.value.toFixed(2)} {t("activation3.checkout.promoDiscount")}
                        </span>
                      );
                      if (b.type === "data") return (
                        <span key={i} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md border border-emerald-200/70 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-medium">
                          <Gift className="w-2.5 h-2.5" /> +{b.value} {t("activation3.checkout.promoData")}
                        </span>
                      );
                      if (b.type === "credit") return (
                        <span key={i} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md border border-emerald-200/70 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-medium">
                          <Gift className="w-2.5 h-2.5" /> {t("activation3.checkout.promoCredit")}
                        </span>
                      );
                      return null;
                    })}
                ...
                <div className="mt-3 space-y-1.5">
                  <div className="flex gap-2">
                    <Input value={promoCode} onChange={(e) => { setPromoCode(e.target.value.toUpperCase()); setPromoError(false); }} placeholder={t("activation3.checkout.promoPlaceholder")} className={cn("flex-1", promoError && "border-destructive")} />
                    <Button type="button" className="shrink-0 bg-primary/10 hover:bg-primary/20 text-foreground border-0 rounded-xl" onClick={() => { if (PROMO_CATALOGUE[promoCode]) { setPromoApplied(true); setPromoError(false); } else setPromoError(true); }}>{t("activation3.checkout.applyPromo")}</Button>
                  </div>
                  <p className="text-[11px] text-muted-foreground">Prototype: try SAVE10 (single discount) or MEGA (discount + data + credit).</p>
                </div>
              ))}
              {promoEnabled && promoError && <p className="text-xs text-destructive mt-1.5">{t("activation3.checkout.promoError")}</p>}
            </section>
            )}
            ...
            {isPostpaidMobile && selectedPlanObj && (
              <div className="flex items-start gap-2.5 rounded-2xl bg-sky-50 dark:bg-sky-500/10 border border-sky-200 dark:border-sky-500/20 p-3.5">
                ...
                <p className="text-xs text-sky-700 dark:text-sky-300 leading-snug">
                  <span className="font-semibold">{t("activation3.checkout.creditLimitNote", { amount: switchPostpaidCreditLimit.toFixed(2) })}</span>{" "}
                  {t("activation3.checkout.creditLimitSub")}
                </p>
              </div>
            )}
            ...
            {isWhitelisted && (
              <div className="rounded-2xl border border-amber-300 bg-amber-50 dark:bg-amber-900/20 dark:border-amber-700 px-4 py-3 flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-amber-700 dark:text-amber-400">{t("activation3.identity.whitelisted.noDeposit")}</p>
                  {isVipNumber ? (
                    <p className="text-[11px] text-amber-600 dark:text-amber-500 mt-0.5">
                      {t("activation3.identity.whitelisted.vipNotice")}
                    </p>
                  ) : (
                    <p className="text-[11px] text-amber-600 dark:text-amber-500 mt-0.5">
                      {t("activation3.identity.whitelisted.freeNotice")}
                    </p>
                  )}
                </div>
              </div>
                  ...
                  <>
                    <div className="space-y-2 pb-3">
                      ...
                      {showNumber && subType === "sim" && numberFee === 0 && isPostpaidMobile && pickedVanityCat && pickedVanityCat.months > 0 && pickedCategoryEligibleFree && vanityCommitment && (
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] text-muted-foreground">{t("activation3.checkout.numberPrice")}</span>
                          <span className="text-xs font-semibold text-emerald-600">{t("activation3.checkout.free")}</span>
                        </div>
                      )}
                      {showDevice && deviceFee > 0 && (
                        <div className="flex items-center justify-between">
                      ...
                      {(planMode === "topup" || isPaygPlan) && topupAmount > 0 && (
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] text-muted-foreground">{t("activation3.checkout.topupLabel")}</span>
                          <span className="text-xs font-semibold text-foreground"><RiyalSymbol /> {topupAmount.toFixed(2)}</span>
                        </div>
                      )}
                      {promoApplied && promoDiscount > 0 && (
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] text-green-600">{t("activation3.checkout.promoLabel")} ({promoCode})</span>
            ...
            {!(isWhitelisted && !isVipNumber) && !(isFulfilment && alreadyPaid) && total > 0 && (
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <p className="text-sm font-semibold text-foreground">{t("activation3.checkout.paymentMethod")} <span className="text-destructive">*</span></p>
                </div>
                <div className="space-y-2">
                  <PayOption icon={CreditCard} label={t("activation3.checkout.dealerWallet")} description={t("activation3.checkout.dealerWalletDesc", { balance: DEALER_WALLET_BALANCE.toFixed(2) })} selected={pay === "card"} disabled={total > DEALER_WALLET_BALANCE} justToppedUp={justToppedUp} onClick={() => setPay("card")}>
                    {total > DEALER_WALLET_BALANCE && (
                      <WalletShortNotice
                        message={t("activation3.checkout.walletShort", { amount: (total - DEALER_WALLET_BALANCE).toFixed(2) })}
                        buttonLabel={t("activation3.checkout.topUpWallet")}
                      />
                    )}
                  </PayOption>
            ...
            {/* Contact Details */}
            <div className="space-y-2">
              <p className="text-sm font-semibold text-foreground px-1">{t("activation3.checkout.contactDetails")}</p>
              <div className="bg-card rounded-2xl p-4 shadow-[var(--card-shadow)] space-y-3 border border-border/60">
                <Field label={emailRequired ? `${t("activation3.checkout.email")} *` : t("activation3.checkout.email")}>
                  <Input
                    value={contactEmail}
                ...
                <Field label={contactNumberRequired ? `${t("activation3.checkout.contactNumber")} *` : t("activation3.checkout.contactNumber")}>
            ...
            {showDelivery && (
              <div className="space-y-2">
                ...
                <div className="bg-card rounded-2xl p-4 shadow-[var(--card-shadow)] space-y-3 border border-border/60">
                  ...
                  <Field label={t("activation3.checkout.addressLine")}>
                    <textarea
                      value={deliveryAddress}
                      onChange={(e) => setDeliveryAddress(e.target.value)}
                      placeholder={t("activation3.subscription.addressPlaceholder")}
                      rows={3}
            ...
            <p className="text-sm font-semibold text-foreground px-1">{t("activation3.checkout.verification")}</p>

            {/* Customer Verification (ID Verification) — first step, always available */}
            <SectionCard title={t("activation3.checkout.customerVerification")} required>
              {customerVerified ? (
            ...
            {showOtp && (
              <SectionCard title={t("activation3.checkout.otp")} required={otpRequired}>
                {otpVerified ? (
            ...
            {showNafith && (
              <SectionCard title={t("activation3.checkout.nafath")} required>
                {nafithVerified ? (
                  ...
                  <>
                    <Button variant="outline" className="w-full bg-primary/10 hover:bg-primary/20 text-foreground border-0 rounded-full disabled:!opacity-100 disabled:!bg-muted disabled:!text-muted-foreground" disabled={!nafithGateOk} onClick={() => setNafithVerifyOpen(true)}>
                      {t("activation3.checkout.nafathVerify")}
                    </Button>
                    {!nafithGateOk && (
                      <p className="text-[11px] text-muted-foreground mt-2">{t("activation3.checkout.nafathLocked")}</p>
                    )}
                  </>
                )}
              </SectionCard>
            ...
            {showPromoCalls && (
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                ...
                >
                  ...
                  <span className="text-sm text-foreground">{t("activation3.checkout.allowPromoCalls")}</span>
                </button>
              </section>
            )}
            ...
            <SignatureBox title={t("activation3.checkout.customerSig")} required value={customerSig} onEdit={() => setSigEditor("customer")} onClear={() => setCustomerSig(null)} />

            {/* Dealer Signature — freely accessible; Payment itself is still gated on all verification steps being done */}
            <SignatureBox title={t("activation3.checkout.dealerSig")} required value={dealerSig} onEdit={() => setSigEditor("dealer")} onClear={() => setDealerSig(null)} />
          </>
        )}
      </div>
        ...
        <div className="max-w-[390px] mx-auto">
          ...
          {SHOW_MISSING_HINT && stepMissing.length > 0 && (
            <p className="text-[11px] text-muted-foreground text-center mb-2 px-2">
              {t("activation3.missing.prefix", { items: joinList(stepMissing, t("activation3.missing.and")) })}
            </p>
          )}
          {step < 2 ? (
            <>
              {step === 1 && (
                <div className="flex items-center justify-center gap-1.5 -mt-0.5 mb-2 px-3.5 py-1 rounded-full bg-primary/5 border border-primary/15 w-fit mx-auto leading-none">
                  ...
                  <span className="text-[12px] text-muted-foreground">{t("activation3.subscription.walletBalanceLabel")}</span>
                  <span className="text-[12px] font-bold text-primary"><RiyalSymbol /> {DEALER_WALLET_BALANCE.toFixed(2)}</span>
                </div>
              )}
              <Button className="w-full h-12 text-sm font-semibold rounded-full" disabled={!canContinue} onClick={onContinue}>{t("activation3.continue")}</Button>
            </>
          ) : (
            <Button className="w-full h-12 text-sm font-semibold rounded-full" disabled={!canPay} onClick={() => setPayConfirmOpen(true)}>
              {total === 0 ? t("activation3.checkout.submit") : <>{t("activation3.checkout.pay")} <RiyalSymbol /> {total.toFixed(2)}</>}
            </Button>
          )}
        </div>
      </div>
      ...
      {qrScanOpen && (
        <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center px-8">
          <button
            ...
            aria-label={t("activation3.verification.cancel")}
          >
          ...
          {qrScanStep === "scanning" ? (
            <>
              ...
              <p className="text-white text-sm font-semibold mt-8 text-center leading-snug">
                {t("activation3.identity.qrNoticeDesc")}
              </p>
              <p className="text-white/50 text-xs mt-2 flex items-center gap-1.5">
                <Loader2 className="w-3.5 h-3.5 animate-spin" /> {t("activation3.identity.qrScanning")}
              </p>
            </>
          ) : (
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-center">
                <h4 className="font-semibold text-white mb-1">{t("activation3.identity.qrFound")}</h4>
                <p className="text-xs text-white/70">{t("activation3.identity.qrFoundNote")}</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* KSA regulation: blocks Continue when Prepaid is picked for a customer who's hit their
          prepaid activation limit — the CTA switches them straight to Basic Postpaid instead
          ...
          <Drawer open={numberPickerOpen} onOpenChange={setNumberPickerOpen}>
            <DrawerContent className="bg-card rounded-t-3xl h-[88vh] flex flex-col">
              <div className="flex items-center justify-between px-5 pt-3 pb-4">
                <h2 className="text-lg font-bold text-foreground">{t("activation3.checkout.numberPickerTitle")}</h2>
              ...
              <div className="px-5 mb-3">
                <div className="relative">
                  <input value={numberSearch} onChange={e => setNumberSearch(e.target.value)} placeholder={t("activation3.checkout.search")} style={{ fontSize: "16px" }} className="w-full h-11 bg-muted/50 rounded-xl ps-4 pe-10 text-base outline-none border border-border/40 rtl:text-right" />
                        ...
                        {freeWithCommitment ? (
                          <span className="flex flex-col items-end">
                            <span className="text-xs font-semibold text-emerald-600">
                              {vanityCat ? t("activation3.vanity.commitmentOn", { months: vanityCat.months }) : t("activation3.vanity.freeWithCommitment")}
                            </span>
                            <span className="text-[11px] text-muted-foreground line-through"><RiyalSymbol /> {(vanityCat?.price ?? fee).toFixed(2)}</span>
                          </span>
                        ) : freePlain ? (
                          <span className="text-sm font-semibold text-muted-foreground">{t("activation3.checkout.free")}</span>
                        ) : fee > 0 ? (
        ...
        return (
          <Dialog open onOpenChange={(o) => !o && setPendingVanityNumber(null)}>
            <DialogContent className="max-w-[320px] rounded-3xl border-0 p-6 text-center [&>button]:hidden">
              ...
              <p className="text-xs text-muted-foreground mb-4">{t("activation3.vanity.choosePrompt")}</p>
              <div className="flex flex-col gap-2">
                ...
                >
                  {t("activation3.vanity.getFreeWithCommitment", { months: cat.months })}
                </button>
                <button
                ...
                >
                  {t("activation3.vanity.payNumberPrice", { price: cat.price.toFixed(2) })}
                </button>
                <button onClick={() => setPendingVanityNumber(null)} className="text-primary text-sm font-medium mt-1">
                  {t("activation3.checkout.cancelBtn")}
                </button>
              </div>
            </DialogContent>
          </Dialog>
      ...
      <Drawer open={nationalityPickerOpen} onOpenChange={(o) => { setNationalityPickerOpen(o); if (!o) setNationalitySearch(""); }}>
        <DrawerContent className="bg-card rounded-t-3xl max-h-[88vh] flex flex-col">
          ...
          <div className="flex items-center justify-between px-5 pt-3 pb-4">
            <h2 className="text-lg font-bold text-foreground">{t("activation3.identity.selectNationality")}</h2>
          ...
          <div className="px-5 mb-3">
            <div className="relative">
              <input
                ...
                placeholder={t("activation3.checkout.search")}
      ...
      <Drawer open={esimInfoOpen} onOpenChange={(o) => { setEsimInfoOpen(o); if (!o) setEsimDeviceSearch(""); }}>
        <DrawerContent className="bg-card rounded-t-3xl max-h-[88vh] flex flex-col">
          ...
          <div className="px-5 pt-3 pb-4">
            <h2 className="text-lg font-bold text-foreground">{t("activation3.checkout.esimDevicesTitle")}</h2>
            <p className="text-xs text-muted-foreground mt-0.5">{t("activation3.checkout.esimDevicesNote")}</p>
          </div>
          ...
          <div className="overflow-y-auto flex-1 px-5 pb-6 pt-3 space-y-4">
            {(() => {
              ...
              return filteredDevices.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground text-center py-6">{t("activation3.checkout.noDevicesFound")}</p>
              );
            })()}
            <p className="text-[11px] text-muted-foreground text-center px-4">{t("activation3.checkout.esimUnlocked")}</p>
          </div>
          <div className="px-5 pb-6 pt-2">
            <Button className="w-full rounded-xl" onClick={() => setEsimInfoOpen(false)}>{t("activation3.checkout.gotIt")}</Button>
          </div>
        </DrawerContent>
      </Drawer>
      ...
      <Drawer open={otpOpen} onOpenChange={setOtpOpen}>
        <DrawerContent className="bg-card rounded-t-3xl border-0 px-5 pb-8 pt-2">
          <div className="flex flex-col items-center gap-4 py-4">
            <h3 className="text-lg font-bold text-foreground">{t("activation3.otpSheet.title")}</h3>
            <p className="text-sm text-muted-foreground text-center px-4">
              {otpError ? t("activation3.otpSheet.errorSubtitle") : t("activation3.otpSheet.subtitle")}
            </p>
            <div className="flex gap-2">
              {otpDigits.map((d, i) => (
                <input
            ...
            <p className="text-xs text-muted-foreground">
              {otpError ? (
                <>
                  {t("activation3.otpSheet.resendLabel")}{" "}
                  <button type="button" onClick={resendOtp} className="text-primary font-semibold">{t("activation3.otpSheet.resend")}</button>
                </>
              ) : otpSecondsLeft > 0 ? (
                <>
                  {t("activation3.otpSheet.noCode")}{" "}
      ...
      <Drawer open={termsOpen} onOpenChange={setTermsOpen}>
        <DrawerContent className="max-h-[85vh]">
          ...
          <DrawerHeader className="text-center">
            <DrawerTitle>{t("activation3.termsSheet.title")}</DrawerTitle>
            <DrawerDescription>{t("activation3.termsSheet.subtitle")}</DrawerDescription>
          </DrawerHeader>
          <div className="overflow-y-auto px-4 py-2 text-sm text-foreground space-y-3 rtl:text-right">
            <p>{t("activation3.termsSheet.p1")}</p>
            <p>{t("activation3.termsSheet.p2")}</p>
            <p>{t("activation3.termsSheet.p3")}</p>
          </div>
          <DrawerFooter className="flex-col gap-3">
            <DrawerClose asChild>
              <Button onClick={() => { setTermsOpen(false); if (termsChain) { setPrivacyOpen(true); } else { setTerms(true); } }} className="w-full h-12 rounded-full">{t("activation3.termsSheet.accept")}</Button>
            </DrawerClose>
            <DrawerClose asChild>
              <button type="button" className="text-sm font-semibold text-primary">{t("activation3.termsSheet.cancel")}</button>
            </DrawerClose>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
      ...
      <Drawer open={privacyOpen} onOpenChange={setPrivacyOpen}>
        <DrawerContent className="max-h-[85vh]">
          ...
          <DrawerHeader className="text-center">
            <DrawerTitle>{t("activation3.privacySheet.title")}</DrawerTitle>
            <DrawerDescription>{t("activation3.privacySheet.subtitle")}</DrawerDescription>
          </DrawerHeader>
          <div className="overflow-y-auto px-4 py-2 text-sm text-foreground space-y-3 rtl:text-right">
            <p>{t("activation3.privacySheet.p1")}</p>
            <p>{t("activation3.privacySheet.p2")}</p>
            <p>{t("activation3.privacySheet.p3")}</p>
          </div>
          <DrawerFooter className="flex-col gap-3">
            <DrawerClose asChild>
              <Button onClick={() => { if (termsChain) { setTerms(true); setTermsChain(false); } }} className="w-full h-12 rounded-full">{t("activation3.privacySheet.close")}</Button>
            </DrawerClose>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
      ...
      <SignaturePadSheet
        open={sigEditor !== null}
        title={sigEditor === "customer" ? t("activation3.checkout.customerSig") : t("activation3.checkout.dealerSig")}
      ...
      <Drawer open={cancelOpen} onOpenChange={(o) => { if (!o) { setCancelOpen(false); setCancelReason(""); setCancelOtherText(""); } }}>
        <DrawerContent className="bg-card rounded-t-3xl border-0 px-5 pb-8 pt-2">
          <DrawerHeader className="text-start px-0 pb-4">
            <DrawerTitle>{t("activation3.cancelSheet.title")}</DrawerTitle>
            <DrawerDescription>{t("activation3.cancelSheet.subtitle")}</DrawerDescription>
          </DrawerHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-foreground">{t("activation3.cancelSheet.reasonLabel")} <span className="text-destructive">*</span></label>
            ...
            {cancelReason === "other" && (
              <div className="space-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
                <label className="text-sm font-semibold text-foreground">{t("activation3.cancelSheet.specify")} <span className="text-destructive">*</span></label>
                <Textarea value={cancelOtherText} onChange={(e) => setCancelOtherText(e.target.value)} placeholder={t("activation3.cancelSheet.specifyPlaceholder")} className="min-h-[100px] px-4 py-3 bg-white border border-border/60 rounded-xl text-sm resize-none rtl:text-right" />
              </div>
            )}
          </div>
          <div className="flex flex-col gap-2 mt-6">
            <Button disabled={!cancelReason || (cancelReason === "other" && !cancelOtherText.trim())} onClick={() => { setCancelOpen(false); setCancelReason(""); setCancelOtherText(""); navigate("/"); }} className="w-full h-11 rounded-full">{t("activation3.cancelSheet.confirm")}</Button>
            <Button variant="outline" onClick={() => { setCancelOpen(false); setCancelReason(""); setCancelOtherText(""); }} className="w-full h-11 rounded-full border-primary text-primary">{t("activation3.cancelSheet.keepEditing")}</Button>
          </div>
        </DrawerContent>
      </Drawer>
    </div>
