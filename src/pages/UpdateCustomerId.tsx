    ...
    <div className="mobile-container min-h-screen bg-background pb-32">
          ...
          <>
            ...
            <Field label={t("updateCustomerId.idNumber")}>
              <Input
                ...
                className={cn("h-12 bg-card border-border rounded-xl", idNumber.trim().length > 0 && !currentIdNumberValid && "border-destructive focus-visible:ring-destructive")}
              />
      ...
      <Drawer open={termsOpen} onOpenChange={setTermsOpen}>
        <DrawerContent className="max-h-[85vh]">
          ...
          <DrawerHeader className="text-center">
            <DrawerTitle>{t("updateCustomerId.termsAndConditions")}</DrawerTitle>
            <DrawerDescription>{t("updateCustomerId.termsDrawerDesc")}</DrawerDescription>
          </DrawerHeader>
          <div className="overflow-y-auto px-4 py-2 text-sm text-foreground space-y-3 rtl:text-right">
            <p>{t("updateCustomerId.termsParagraph1")}</p>
            <p>{t("updateCustomerId.termsParagraph2")}</p>
            <p>{t("updateCustomerId.termsParagraph3")}</p>
          </div>
      ...
      <Drawer open={privacyOpen} onOpenChange={setPrivacyOpen}>
        <DrawerContent className="max-h-[85vh]">
          ...
          <DrawerHeader className="text-center">
            <DrawerTitle>{t("updateCustomerId.privacyPolicy")}</DrawerTitle>
            <DrawerDescription>{t("updateCustomerId.privacyDrawerDesc")}</DrawerDescription>
          </DrawerHeader>
          <div className="overflow-y-auto px-4 py-2 text-sm text-foreground space-y-3 rtl:text-right">
            <p>{t("updateCustomerId.privacyParagraph1")}</p>
            <p>{t("updateCustomerId.privacyParagraph2")}</p>
            <p>{t("updateCustomerId.privacyParagraph3")}</p>
          </div>
          <DrawerFooter className="flex-col gap-3">
            <DrawerClose asChild>
              <Button onClick={() => { if (termsChain) { setTermsAccepted(true); setTermsChain(false); } }} className="w-full h-12 rounded-full">
                {t("updateCustomerId.close")}
              </Button>
            </DrawerClose>
          </DrawerFooter>
        </DrawerContent>
      ...
      <Drawer open={cancelOpen} onOpenChange={(o) => { setCancelOpen(o); if (!o) { setCancelReason(""); setCancelOtherText(""); } }}>
        <DrawerContent className="bg-card rounded-t-3xl border-0 px-5 pb-8 pt-2">
          <DrawerHeader className="text-start px-0 pb-4">
            <DrawerTitle>{t("updateCustomerId.cancelSheet.title")}</DrawerTitle>
            <DrawerDescription>{t("updateCustomerId.cancelSheet.subtitle")}</DrawerDescription>
          </DrawerHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-sm font-semibold text-foreground">{t("updateCustomerId.cancelSheet.reasonLabel")} <span className="text-destructive">*</span></label>
              <Select value={cancelReason} onValueChange={setCancelReason}>
                <SelectTrigger className="h-12 px-4 bg-white border border-border/60 rounded-xl text-sm">
                  <SelectValue placeholder={t("updateCustomerId.cancelSheet.selectReason")} />
                </SelectTrigger>
                <SelectContent className="bg-card border-border/60 rounded-xl">
                  <SelectItem value="customer-changed-mind">{t("updateCustomerId.cancelSheet.reasons.customerChangedMind")}</SelectItem>
                  <SelectItem value="missing-documents">{t("updateCustomerId.cancelSheet.reasons.missingDocuments")}</SelectItem>
                  <SelectItem value="id-mismatch">{t("updateCustomerId.cancelSheet.reasons.idMismatch")}</SelectItem>
                  <SelectItem value="system-issue">{t("updateCustomerId.cancelSheet.reasons.systemIssue")}</SelectItem>
                  <SelectItem value="other">{t("updateCustomerId.cancelSheet.reasons.other")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            {cancelReason === "other" && (
              <div className="space-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
                <label className="text-sm font-semibold text-foreground">{t("updateCustomerId.cancelSheet.specify")} <span className="text-destructive">*</span></label>
                <Textarea value={cancelOtherText} onChange={(e) => setCancelOtherText(e.target.value)} placeholder={t("updateCustomerId.cancelSheet.specifyPlaceholder")} className="min-h-[100px] px-4 py-3 bg-white border border-border/60 rounded-xl text-sm resize-none rtl:text-right" />
              </div>
            )}
          </div>
          <div className="flex flex-col gap-2 mt-6">
            <Button disabled={!cancelReason || (cancelReason === "other" && !cancelOtherText.trim())} onClick={() => { setCancelOpen(false); setCancelReason(""); setCancelOtherText(""); resetAll(); navigate("/"); }} className="w-full h-11 rounded-full">{t("updateCustomerId.cancelSheet.confirm")}</Button>
            <Button variant="outline" onClick={() => { setCancelOpen(false); setCancelReason(""); setCancelOtherText(""); }} className="w-full h-11 rounded-full border-primary text-primary">{t("updateCustomerId.cancelSheet.keepEditing")}</Button>
          </div>
        </DrawerContent>
      </Drawer>
