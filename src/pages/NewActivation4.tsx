...
const NewActivation4 = () => {
  ...
  const step2Missing = useMemo(() => {
    ...
    if (pay === "card" && total > DEALER_WALLET_BALANCE) missing.push(t("activation4.missing.walletBalance"));
    return missing;
      ...
      <div className="px-4 space-y-4">
        ...
        {step === 1 && (
          <>
                            ...
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              {showCommitted && pickedVanityCat ? (
                                ...
                                <span className="text-[11px] font-semibold text-foreground">
                                  {pickedVanityCat ? <><RiyalSymbol /> {pickedVanityCat.price.toFixed(2)}</> : (tab.fee ? <><RiyalSymbol /> {tab.fee.toFixed(2)}</> : t("activation4.checkout.free"))}
                                </span>
                              )}
                            </div>
                            {showCommitted && pickedVanityCat && (
            ...
            {showPlanTypeChips && (
              <div className="space-y-2">
                <h3 className="text-sm font-semibold text-foreground">{t("activation4.subscription.planTypesTitle")}</h3>
                <div
                {...planChipsDragScroll}
            ...
            <div className="flex items-center gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  placeholder={t("activation4.subscription.searchPlans")}
              ...
              >
                {t("activation4.subscription.allPlansBtn")}
            ...
            {isPaygPlan && selectedPlanObj && (() => {
              ...
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                ...
                <p className="text-[11px] text-muted-foreground mb-3">
                  {hasSelection
                    ? <><RiyalSymbol /> {inclVat.toFixed(2)} {t("activation4.subscription.topupInclVat")}</>
              ...
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                ...
                {subType === "sim" ? (
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[11px] font-semibold" style={{ color: tab.color ?? undefined }}>{t(`activation.subscription.numberTabs.${tab.value}`, tab.label)}</span>
                            <span className="text-[11px] text-muted-foreground">·</span>
                            <span className="text-[11px] font-semibold text-foreground">{tab.fee ? <><RiyalSymbol /> {tab.fee.toFixed(2)}</> : t("activation4.checkout.free")}</span>
                          </div>
                        );
                      })()}
                      */}
                        ...
                        >
                          ...
                          <span className="flex-1 min-w-0">
                            ...
                            <p className="text-[11px] text-muted-foreground">
                              {vanityCommitment
                                ? t("activation4.vanity.commitmentOn", { months: pickedVanityCat.months })
                                : t("activation4.vanity.commitmentOff", { price: pickedVanityCat.price.toFixed(2) })}
                            </p>
                          </span>
                  ...
                  <div className="space-y-3">
                    <Field label={t("activation4.subscription.portNumber")}><PhoneNumberInput value={portNumber} onChange={setPortNumber} /></Field>
                      ...
                      pickedCategoryEligibleFree ? (
                        <div className="mt-3 rounded-xl bg-muted/40 border border-border/60 p-3 space-y-2.5">
                          ...
                          {vanityCommitment
                            ...
                            : <p className="text-[11px] font-semibold text-foreground"><RiyalSymbol /> {pickedVanityCat.price.toFixed(2)} · {t("activation4.vanity.noCommitment")}</p>}
                        </div>
                      ) : (
                        <div className="mt-3 rounded-xl bg-muted/40 border border-border/60 p-3 space-y-1">
                          <p className="text-[13px] font-semibold text-foreground">{t(`activation.vanity.categories.${pickedVanityCat.key}`)} · {t(`activation.vanity.tiers.${pickedVanityCat.tier}`)}</p>
        ...
        {step === 2 && (
          <>
            ...
            <section className="bg-card rounded-2xl p-4 shadow-sm">
              ...
              {!isFriendi && <SummaryRow label={t("activation4.subscription.lineTypeTitle")} value={lineType === "data" ? t("activation4.subscription.lineData") : t("activation4.subscription.lineMobile")} />}
              ...
              {(planMode === "topup" || isPaygPlan) && topupAmount > 0 && <SummaryRow label={t("activation4.checkout.topupValue")} value={<><RiyalSymbol /> {topupAmount.toFixed(2)}</>} />}
                    ...
                    {activePromo.benefits.map((b, i) => {
                      if (b.type === "discount") return (
                        <span key={i} className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md border border-emerald-200/70 dark:border-emerald-700/40 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 text-[10px] font-medium">
                          <RiyalSymbol className="w-2.5 h-2.5" /> {b.value.toFixed(2)} {t("activation4.checkout.promoDiscount")}
                        </span>
                      );
              ...
              {isFulfilment && alreadyPaid ? (() => {
                ...
                <>
                  <div className="space-y-2 pb-3">
                    ...
                    {selectedPlanObj && (
                      <div className="flex items-center justify-between">
                        <span className="text-[11px] text-muted-foreground">{payType === "postpaid" ? t("activation4.checkout.deposit") : t("activation4.checkout.planLabel")}</span>
              ...
              isWhitelisted && !isVipNumber ? (
                <>
                  <div className="space-y-2 pb-3">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] text-muted-foreground">{planMode === "plan" ? (payType === "postpaid" ? t("activation4.checkout.deposit") : t("activation4.checkout.planLabel")) : t("activation4.checkout.topupLabel")}</span>
                      <span className="text-xs font-semibold text-amber-600">{t("activation4.checkout.waived")}</span>
                    </div>
                  </div>
                  <div className="border-t border-border/60 space-y-2 py-3">
                ...
                isWhitelisted && isVipNumber ? (() => {
                  ...
                  <>
                    <div className="space-y-2 pb-3">
                      ...
                      {selectedPlanObj && (
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] text-muted-foreground">{isPostpaidDeposit ? t("activation4.checkout.deposit") : t("activation4.checkout.planLabel")}</span>
            ...
            {!(isWhitelisted && !isVipNumber) && !(isFulfilment && alreadyPaid) && total > 0 && (
              <section className="bg-card rounded-2xl p-4 shadow-sm">
                ...
                <div className="space-y-2">
                  <PayOption icon={CreditCard} label={t("activation4.checkout.dealerWallet")} description={t("activation4.checkout.dealerWalletDesc", { balance: DEALER_WALLET_BALANCE.toFixed(2) })} selected={pay === "card"} onClick={() => setPay("card")} />
        ...
        <div className="max-w-[390px] mx-auto">
          ...
          {step < 2 ? (
            ...
            <Button className="w-full h-12 text-sm font-semibold rounded-full" disabled={!canPay} onClick={() => setPayConfirmOpen(true)}>
              {total === 0 ? t("activation4.checkout.submit") : <>{t("activation4.checkout.pay")} <RiyalSymbol /> {total.toFixed(2)}</>}
            </Button>
          )}
        </div>
      </div>
          ...
          <Dialog open onOpenChange={(o) => !o && setPendingVanityNumber(null)}>
            <DialogContent className="max-w-[320px] rounded-3xl border-0 p-6 text-center [&>button]:hidden">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  {t("activation4.vanity.payNumberPrice", { price: cat.price.toFixed(2) })}
                </button>
            ...
            <div className="space-y-2">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  navigate("/");
                  toast({
                    title: t("activation4.success.shareSuccessTitle"),
                    description: t("activation4.success.shareSuccessDesc", { target: shareValue }),
                  });
                }}
              >
