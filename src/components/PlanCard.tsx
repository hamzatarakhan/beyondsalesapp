...
// Small inline app icon (real SVG, scaled down)
const AppIconColors: string[] = ["#25D366", "#FF0000", "linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)", "#000000", "#000000", "#1877F2", "#FFFC00", "#26A5E4"];
const AppIconSvgs: string[] = [
  ...
  <button onClick={onClick} className="active:opacity-70 shrink-0">
    <span className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-sky-50 dark:bg-sky-500/10 text-sky-600 text-[10px] font-semibold pointer-events-none">
      <span className="flex -space-x-1">
        {[{ bg: "#25D366" }, { bg: "#FF0000" }, { bg: "linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)" }].map((app, i) => (
          <span key={i} className="w-4 h-4 rounded-full border border-card shrink-0" style={{ background: app.bg }} />
        ))}
      </span>
...
const FlagChip = ({ onClick }: { onClick: () => void }) => (
  <button onClick={onClick} className="active:opacity-70 shrink-0">
    <span className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-sky-50 dark:bg-sky-500/10 text-sky-600 text-[10px] font-semibold pointer-events-none">
      <span className="flex -space-x-0.5 text-[11px]">
        {COUNTRIES.slice(0, PREVIEW_COUNT).map((c) => c.flag).join("")}
      </span>
